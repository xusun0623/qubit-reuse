...
class QRMapCompiler:
    ...
    def __init__(self, quantum_circuit: QuantumCircuit, quantum_chip: QuantumChip,
        ...
        self.idling_threshold = self.params.get(
            'idling_threshold', None)  # 空闲时间阈值，用于判断何时可以重用量子比特
        self.use_most_qubit_rule = self.params.get(
            'use_most_qubit_rule', True)  # 是否使用最多量子位规则选择pivot列
    ...
    def extract_two_qubit_gates(self):
        ...
        for instruction in self.quantum_circuit.data:
            ...
            qubits = [self.quantum_circuit.qubits.index(
                q) for q in instruction.qubits]
    ...
    def apply_tapering(self):
        ...
        while True:
            ...
            direction = self._select_direction(
                pivot, working_map['array'].shape[1])
    ...
    def _select_pivot(self, qr_map):
        ...
        if self.use_most_qubit_rule:
            # 选择门数量最多的列
            gate_counts = [np.count_nonzero(array[:, col])
    ...
    def _try_move_column(self, qr_map, moving_col, pivot, direction, trigger_row):
        ...
        # 生成移动偏好顺序
        preferences = self._generate_move_preferences(
            moving_col, pivot, direction)
    ...
    def restore_to_circuit(self):
        ...
        num_physical_qubits = max(qubit_mapping.values(
        )) + 1 if qubit_mapping else self.quantum_circuit.num_qubits
        ...
        for gate_idx, gate_info in enumerate(self.optimized_map['gate_info']):
            ...
            if len(physical_qubits) != len(set(physical_qubits)):
                raise ValueError(
                    f"门 {gate_idx} 的物理量子比特存在重复: {physical_qubits}")
            ...
            for qubit, single_gates in gate_info['single_qubit_gates'].items():
                ...
                for single_gate in single_gates:
                    optimized_circuit.append(
                        single_gate['gate'], [physical_qubit])

            # 添加两量子比特门
            optimized_circuit.append(gate_info['gate'], physical_qubits)

            # 检查是否需要插入测量和重置（量子位重用点）
            self._insert_measure_reset_if_needed(
                optimized_circuit, gate_idx, qubit_mapping)

        # 计算量子位重用次数
        self.qubit_reuse_count = self._calculate_qubit_reuse()

        print(
            f"电路恢复完成: 物理量子位数={num_physical_qubits}, 重用次数={self.qubit_reuse_count}")
        return optimized_circuit
